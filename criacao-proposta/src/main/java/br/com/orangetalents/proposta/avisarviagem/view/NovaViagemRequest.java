...
public class NovaViagemRequest {
    ...
    public NovaViagemRequest(@NotBlank String destino, @NotNull @Future @JsonFormat(pattern = "dd/MM/yyyy") LocalDate dataTermino) {
        this.destino = destino;
        this.dataTermino = LocalDate.parse(dataTermino.format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
    ...
    public AvisoViagem toModel(Cartao cartao, String ip, String userAgent) {
        return new AvisoViagem(this.dataTermino, this.destino, cartao, ip, userAgent, this.instante = LocalDateTime.now());
    }
