...
public class BloqueioController implements SelecionaCartao {
  ...
  public ResponseEntity<RedirectView> selecionaCartao(
      @RequestBody @Valid CartaoRequest cartaoRequest, UriComponentsBuilder uriComponentsBuilder) {
  ...
  @Transactional
  public ResponseEntity<?> bloqueia(
      @PathVariable("numeroCartao") String numeroCartao, HttpServletRequest httpServletRequest) {
    ...
    if (bloqueio != null) {
       ...
       * */
      cartao.alteraStatusCartao(
          bloqueioSistemaExterno.bloqueiaCartao(
              cartao.getId(), new BloqueioCartaoRequest("api-carol")));
      em.merge(cartao);
